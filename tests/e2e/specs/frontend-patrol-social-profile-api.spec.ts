...
test.describe("frontend M4: perfil social remoto da patrulha", () => {
  test("exibe dados remotos de monitor, submonitor e membros", async ({
    ...
    await page.route(`**/patrulha/${patrolId}/social-profile`, async (route) => {
      await route.fulfill({
        status: 200,
        ...
        body: JSON.stringify({
          ...
          submonitorId: "cccccccc-cccc-cccc-cccc-cccccccccccc",
    ...
    await expect(profileSummary.getByText(/Patrulha Lobo Remota/i)).toBeVisible();
    await expect(profileSummary.getByText(/Monitor: Monitor Remoto/i)).toBeVisible();
    await expect(
      profileSummary.getByText(/Submonitor: Submonitor Remoto/i),
    ).toBeVisible();
    await expect(profileSummary.getByText(/Membros cadastrados: 3/i)).toBeVisible();
    await expect(
      profileSummary.getByText(/Membros recentes: Membro A, Membro B/i),
    ).toBeVisible();
  });

  test("mantém resumo social local quando API retorna 404", async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem(
        "auth-store",
        JSON.stringify({
